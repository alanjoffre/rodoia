...
| Arquivo | O que faz | Funções-chave |
|---|---|---|
| `mlops/gate.py` | Gate de avaliação: regressão de métrica falha o CI (12 portões) | `avaliar`, `GATES`, `_acessar`, `_passou` |
...
- **Gate de qualidade:** `src/rodoia/mlops/gate.py` (pisos por métrica, 12 portões) e o CI em `.github/workflows/ci.yml`.
...
- **Testes:** `tests/test_*.py` (155 testes; fundamentos que exigem torch são pulados no CI via `tests/conftest.py`).
