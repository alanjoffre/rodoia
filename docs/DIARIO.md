...
100. **🔬 Testei as duas explicações para a cobertura baixa — e as duas caíram.** O sistema responde só 39% das perguntas que *tinham* resposta. Parece o pior resultado do projeto, mas os números dizem outra coisa: **62% estava disponível** e foi **recusado**, porque custaria passar de 1,3% para **10% de invenção** — numa leitura de contrato, uma cláusula inventada a cada dez respostas é passivo jurídico. **É um ponto de operação escolhido, não um fracasso.** O que fica aberto é subir a cobertura *sem* inventar mais. Explicação (1): *"o contexto entregue ao modelo é ruim"* — e de fato a avaliação parava no buscador intermediário, ignorando o melhor. Liguei o melhor, o teto subiu 5,6 pontos, e a geração **não aproveitou nada** (teste pareado, p = 0,36). Explicação (2): *"o detector de recusa erra"* — erra mesmo: achei uma resposta que começa com **"YES"** e cita a cláusula, contada como recusa. Mas mesmo supondo que **todos** os casos suspeitos fossem erro, a cobertura iria só a 45%. **O número ruim é real**, e agora com as duas hipóteses eliminadas por experimento em vez de por opinião.
