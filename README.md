...
![Tests](https://img.shields.io/badge/testes-289%20passando-brightgreen.svg)
...
**Status: projeto encerrado ✅ · 7 fases (0–6) entregues · 289 testes · gate 30/30 no CI** — cada fase é um marco publicável, testado e documentado antes de a próxima começar, e cada número abaixo aponta para um relatório versionado com carimbo de proveniência.
...
> ⚠️ **O ponto de operação, e o que fica em aberto.** O sistema opera a **1,3% de alucinação com 38,7% de cobertura** — o extremo conservador de uma curva medida. Cobertura de **62% está disponível e foi recusada**: custaria subir a invenção para **10%**, e num sistema que lê contrato uma cláusula inventada a cada dez respostas é passivo jurídico, não inconveniência. O que fica genuinamente aberto é **elevar a cobertura sem pagar em alucinação** — e as duas explicações mais plausíveis foram **testadas e caíram**: o contexto não é o gargalo (o rerank subiu o teto 5,6 pp e a cobertura não se moveu, p = 0,361) e o detector de abstenção erra pouco demais (teto otimista 0,45). Sobra o *few-shot*, **não medido**. Também em aberto: **deploy em nuvem não executado** (runbook pronto, incluindo a trilha AWS) e **DVC sem remote**. Detalhe em [docs/17 §13.8](docs/17_fase6_escala.md).
...
|---|---|---|
| Python sólido (async, tipagem, produção) | Todas | **`mypy --strict` no núcleo servido, bloqueante no CI** (scripts de pesquisa fora por override declarado — [docs/16](docs/16_fase5_mlops.md) §2.1) · 289 testes (272 no CI) · `async` nos endpoints |
...
O diferencial não são os números altos — é **o rigor ter corrigido os próprios números**. Dezesseis momentos em que a avaliação honesta mudou a conclusão:
...
- **Fase 6 — duas hipóteses sobre a cobertura, as duas testadas e as duas caídas.** A cobertura de 0,387 é o número em aberto do projeto — e é **um ponto de operação, não um fracasso**: os quatro pontos medidos mostram que 62% de cobertura está disponível ao custo de subir a alucinação de 1,3% para 10%, e essa troca foi **recusada** (num sistema que lê contrato, uma cláusula inventada a cada dez respostas é passivo jurídico). O que se quer é elevar a cobertura **sem** pagar em alucinação. (1) *"O contexto está pobre"* — e de fato a avaliação de geração parava no **híbrido**, ignorando o rerank, então a §13.4 comparava a geração com um teto (0,713) que a própria fase já superara (0,769). Liguei o rerank e refiz **pareado**: cobertura 0,387 → 0,427, **p = 0,361 — nada significativo**. Elevar o teto em 5,6 pp não moveu a geração, o que converte *"o gargalo é o gerador"* de afirmação em **experimento**. (2) *"O detector de abstenção erra"* — erra mesmo: uma resposta que começa com **"YES"** e cita a seção estava contada como recusa. Mas no cenário mais generoso possível a cobertura vai a 0,453. **O número fraco é real, não artefato.** Fica em aberto, com duas explicações eliminadas e o custo de cada uma medido — não com "falta polir".
...
[MIT](LICENSE) para o código. Dados e modelos de terceiros seguem as suas próprias licenças, atribuídas em [`NOTICE`](NOTICE) — inclusive o **CUAD**, sob **CC BY 4.0**, que exige atribuição ao [The Atticus Project](https://www.atticusprojectai.org/cuad) e a citação de Hendrycks et al. (NeurIPS 2021).

## 🏁 Encerramento

O projeto está **fechado** em 28 de julho de 2026. Sete fases entregues, **100 passos** registrados no [diário](docs/DIARIO.md), **289 testes** e **gate 30/30** bloqueando o CI.
...
Os itens em aberto estão no topo desta página e em [docs/HISTORIA.md § Estado final](docs/HISTORIA.md), não escondidos aqui embaixo.

---

<div align="center">

<sub>Encerrado em 28 de julho de 2026 · 7 fases · 100 passos · 289 testes · gate 30/30 · custo de API R$0.</sub>
