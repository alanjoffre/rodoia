...
> **🔗 Demo ao vivo (grátis):** busca semântica sobre a regulação da ANTT rodando **no navegador**
> (E5 via Transformers.js) — [**huggingface.co/spaces/alanjoffre/rodoia-rag**](https://huggingface.co/spaces/alanjoffre/rodoia-rag).
...
| Avaliação de LLMs (LLM-as-judge, guardrails, hallucination) | Fase 1 + 2 + 4 | LLM-as-judge **independente** + faithfulness/relevancy + precisão de citação (F1) · **κ humano inter-anotador 0,86** [IC95 0,65–1,00] (relevância) + **0,92** [0,79–1,00] (rótulo-gold de fonte do hit@5, n=50) — 2 anotadores independentes + **banca de 3 juízes LLM com κ de Fleiss** · juiz pareado c/ controle de viés (F2) · guardrails |
...
| **5** | MLOps, Cloud e operação | ✅ concluída ([docs/16](docs/16_fase5_mlops.md)) — **gate de avaliação** (regressão de métrica reprova o CI) · **GitHub Actions** (lint+testes+gate) · **MLflow** (sqlite) + **DVC** · **container** (Dockerfile+compose) · **drift** por PSI (0,005, estável) · **demo pública NO AR** ([huggingface.co/spaces/alanjoffre/rodoia-rag](https://huggingface.co/spaces/alanjoffre/rodoia-rag)) — retrieval E5 **no navegador** via Transformers.js, R$0 (o HF passou a cobrar Gradio/Docker → [Static Space client-side](deploy/hf_space_static/README.md)) · **deploy em cloud = runbook** (Cloud Run justificado, não executado por decisão de orçamento) |
