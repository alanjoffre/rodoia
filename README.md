...
| **5** | MLOps, Cloud e operação | ✅ concluída ([docs/16](docs/16_fase5_mlops.md)) — **gate de avaliação** (regressão de métrica reprova o CI) · **GitHub Actions** (lint+testes+gate) · **MLflow** (sqlite) + **DVC** · **container** (Dockerfile+compose) · **drift** por PSI (0,005, estável) · **demo pública grátis** pronta — **[Static Space client-side](deploy/hf_space_static/README.md)** (retrieval E5 no navegador via Transformers.js, R$0; o HF passou a cobrar por Gradio/Docker) · **deploy em cloud = runbook** (Cloud Run justificado, não executado por decisão de orçamento) |
